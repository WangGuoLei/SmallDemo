...
@interface CollectionViewSort ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation CollectionViewSort
...
- (void)viewDidLoad {
    ...
    for (int i=0; i<15; i++) {
        [self.imagesSectionOneArray addObject:[NSString stringWithFormat:@"%d",i%10+1+100]];
    }
    for (int i=21; i<30; i++) {
        [self.imagesSectionTwoArray addObject:[NSString stringWithFormat:@"%d",i+100]];
    }
    ...
    _mineCollection = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 64.0, kWidth, kHeight-64.0) collectionViewLayout:layout];
...
{
    return CGSizeMake((kWidth - 25.0) / 4.0, (kWidth - 25.0) / 4.0);
}

- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section
{
    return CGSizeMake(kWidth, 44.0);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        cell.cellImage.image = [UIImage imageNamed:self.imagesSectionOneArray[indexPath.item]];
        if (_sectionOneIsSort && indexPath.item + 1 == self.imagesSectionOneArray.count) {
            cell.hidden = YES;
        }
    } else {
        cell.cellImage.image = [UIImage imageNamed:self.imagesSectionTwoArray[indexPath.item]];
        if (_sectionTwoIsSort && indexPath.item + 1 == self.imagesSectionTwoArray.count) {
            cell.hidden = YES;
        }
    }
